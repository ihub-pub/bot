...
module.exports.autoMilestone = app => {
    ...
    app.on("pull_request", async context => {
        ...
        let milestone = (await context.octokit.rest.issues.listMilestones({
            owner: owner,
            repo: repo,
            state: "open"
        })).data
        info(context, `milestone ${milestone}`)
        milestone = milestone.filter(m => new RegExp(/\d+(.\d+)*/).test(m.title))
        info(context, `milestone ${milestone}`)
